...
const ApprovedComplaintsTable = ({
  ...
  // ---- helpers
  const pick = (obj, keys) =>
    Object.fromEntries(keys.map((k) => [k, obj?.[k]]).filter(([, v]) => v !== undefined && v !== ""));

  // Only the fields this component cares about (avoid cross-tab leaks)
  const APPROVED_ALLOWED_KEYS = [
    "bankName",
    "branchCode",
    "branchName",
    "engineerName",
    ...
    "inPool",
    "hasReport",
    // approved date fields for this tab:
    "approvedDate",
    "approvedDateFrom",
    "approvedDateTo",
    // export only:
  ...
  const fetchComplaints = async () => {
    ...
    try {
      ...
      const { reportType, ...filtersForApi } = shared; // never send reportType to list API

      const params = {
        page: currentPage,
        size: pageSize,
        status, // ⬅️ local status only for this table
        ...filtersForApi,
      };
      ...
      try {
        ...
        setBranchGroups((prevGroups) => {
          ...
          const newGroups = prevGroups.map((group) => {
            ...
            if (gKey === groupKey) {
              found = true;
              return { ...group, complaints: [newComplaint, ...(group.complaints || [])] };
            }
            return group;
          });
          if (!found) {
  ...
  const handleGenerateReport = async (passedFilters) => {
    ...
    try {
      setLoading(true);
      const all = await fetchAllComplaintsForExport();
      await generateExcelReport(all, status, API_BASE_URL, uniqueCities, filtersToUse.reportType);
    } catch (err) {
  ...
  // Highlight if aging days from approvedDate > 3
  const shouldHighlightRow = (complaint) => {
    if (!complaint.approvedDate) return false;
    const diff = Math.ceil((new Date() - new Date(complaint.approvedDate)) / (1000 * 60 * 60 * 24));
    return diff > 3;
  };
  ...
  // --- Serial starts at backend-provided offset ---
  ...
  return (
    <div>
      ...
      {selectedComplaintId ? (
        <div>
          <button onClick={handleBackToTable} className="back-button" style={{ marginBottom: 10 }}>
            Back to Table
          </button>
        ...
        <>
              ...
              {loading ? (
                ...
                branchGroups.map((group, groupIdx) => (
                  <React.Fragment key={group.bankName + group.branchCode + groupIdx}>
                    <tr className="group-header-row">
                      <td colSpan="22" className="group-header-row-data" style={{ fontWeight: "bold", backgroundColor: "#d6eaf8" }}>
                    ...
                    {(group.complaints || []).map((complaint) => {
                      ...
                      return (
                        ...
                        >
                          <td>{serialCounter}</td>
                          <td className={getStatusClass(complaint.complaintStatus)}>{complaint.complaintStatus}</td>
                          <td className={getCourierStatusClass(complaint.courierStatus)}>{complaint.courierStatus || "N/A"}</td>
                          <td>
                            <button
                          ...
                          <td>{calculateAgingDays(complaint.approvedDate)}</td>
                ...
                <tr>
                  <td colSpan="22" style={{ textAlign: "center" }}>
                    No approved complaints to display
                  </td>
                </tr>
              )}
            </tbody>
          ...
          {totalPages > 1 && (
            <div className="pagination-container">
              <button className="page-button" onClick={() => setCurrentPage(currentPage - 1)} disabled={currentPage === 0}>
                &lt; Previous
              </button>
...
export default ApprovedComplaintsTable;
