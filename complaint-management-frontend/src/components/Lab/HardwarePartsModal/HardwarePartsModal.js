...
const HardwarePartsModal = ({
  ...
  const handleAssignEngineer = async (part, engineerUsername) => {
    ...
    await axios.put(
      `${API_BASE_URL}/hardware-logs/${logId}/parts/${part.id}`,
      {
        assignedEngineer: engineerUsername || null,
        hardwareName: part.hardwareName,
      },
      { withCredentials: true }
    );
  ...
  const handleMarkRepaired = async (part) => {
    setActioningPartId(part.id);
    await axios.patch(
      `${API_BASE_URL}/hardware-logs/parts/${part.id}/repaired`,
      {},
      { withCredentials: true }
    );
    await refreshParts();
  ...
  const handleMarkNotRepairable = async (part) => {
    setActioningPartId(part.id);
    await axios.patch(
      `${API_BASE_URL}/hardware-logs/parts/${part.id}/not-repairable`,
      {},
      { withCredentials: true }
    );
    await refreshParts();
              ...
              if (mode === "lab") {
                ...
                return (
                  <li key={part.id} className="part-row-card">
                    ...
                    {/* Only Qamar can assign engineer after part accepted but before assignment */}
                    {!part.assignedEngineer &&
                      ...
                      isQamar && (
                        <select
                          ...
                          onChange={(e) =>
                            handleAssignEngineer(part, e.target.value)
                          }
                          disabled={actioningPartId === part.id}
                        >
                    ...
                    {/* Only assigned engineer sees accept if not yet availableWithEngineer */}
                    {isAssigned && !part.availableWithEngineer && isMine && (
                      <button
                        className="accept-btn"
                        onClick={() => handleAccept(part)}
                        disabled={actioningPartId === part.id}
                    ...
                    {/* Only assigned engineer can mark repair status */}
                    {part.availableWithEngineer &&
                      isAssigned &&
                      isMine &&
                      !part.repaired &&
